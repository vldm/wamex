...
fn parse_any_node(input: &str) -> IResult<&str, FlatEntityRef> {
...
fn parse_oneline_deps(
    input: &str,
) -> IResult<&str, HashMap<FlatEntityRef, HashSet<FlatEntityRef>>> {
    if input.trim().is_empty() {
        return Ok((input, HashMap::new()));
    }

    let (input, first_node) = parse_any_node(input)?;
    let mut nodes: HashMap<FlatEntityRef, HashSet<FlatEntityRef>> = HashMap::new();
...
pub fn parse_deps(input: &str) -> IResult<&str, HashMap<FlatEntityRef, HashSet<FlatEntityRef>>> {
    let mut deps: HashMap<FlatEntityRef, HashSet<FlatEntityRef>> = HashMap::new();
...
pub fn parse_list(input: &str) -> IResult<&str, Vec<FlatEntityRef>> {
