...
pub type DepSet<T = FlatEntityRef> = BTreeSet<T>;
pub type DepMiniSet<T = FlatEntityRef> = wamex_types::map_vec::MiniSet<T>;
...
#[derive(Clone)]
pub struct DepGraph {
    // Snapshot is a part used to recover information about symbol kind.
    // And convert ErasedEntityRef/EntityKind to FlatEntityRef.
    snapshot: EntitiesSnapshot,
    // TODO: use VecMap kind of structure for better performance
    nodes: GappedMap<FlatEntityRef, SymbolStructure>,
}
impl DepGraph {
    ...
    pub fn for_testing() -> Self {
        Self {
            nodes: GappedMap::new(),
            snapshot: EntitiesSnapshot::for_testing(),
        }
    }
    ...
    #[cfg(test)]
    pub(crate) fn insert_child(&mut self, parent: FlatEntityRef, child: FlatEntityRef) {
...
pub fn get_dependencies(info: &LoadedFile) -> anyhow::Result<DepGraph> {
    ...
    for (entity, relocs) in info.relocs.iter_relocs() {
        ...
        let id = deps.snapshot.pack_ref(entity);
        ...
        deps.nodes
            // iter_relocs provide entity only once, but it might be already insterted as child of other node,
            // thats why we avoid insert and use entry api
            .entry(id)
            .or_insert_with(SymbolStructure::default)
            .childs = childs;
    }
...
impl<Id> NamedGraph<Id> {
    ...
    fn collect_visited_by(modules: &[NamedGraph<Id>]) -> BTreeMap<FlatEntityRef, DepSet<usize>> {
        let mut visited_by: BTreeMap<FlatEntityRef, DepSet<usize>> = BTreeMap::new();
...
#[cfg(test)]
mod tests {
    use std::{cell::LazyCell, fs::File, io::Write};
    ...
    impl DepListExt for DepMiniSet {
        ...
        fn print(&self, title: &str, info: &Module, graph: &DepGraph) {
            let tmp = self.iter().copied().collect::<DepSet>();
            print_deps_inner(title, info, &tmp, graph);
        }
    }
    ...
    1    -> 307
        ...
        dbg!(&shared_entries);
        let node = FlatEntityRef::from_u32(1417);
    ...
    "#;
        ...
        assert!(first.exports.contains(&FlatEntityRef::from_u32(101)));
        assert!(first.shared_deps.contains(&FlatEntityRef::from_u32(101)));
        assert!(first.shared_deps.contains(&FlatEntityRef::from_u32(102)));
        ...
        assert!(!second.imports.contains(&FlatEntityRef::from_u32(100)));
        assert!(second.exports.contains(&FlatEntityRef::from_u32(100)));
        assert!(second.shared_deps.contains(&FlatEntityRef::from_u32(100)));
        // 101 are imported only
        assert!(second.imports.contains(&FlatEntityRef::from_u32(101)));
        assert!(!second.exports.contains(&FlatEntityRef::from_u32(101)));
        assert!(!second.shared_deps.contains(&FlatEntityRef::from_u32(101)));
    }
