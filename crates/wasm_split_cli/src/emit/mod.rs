...
struct OutputFunction {
    ...
    // Relocation entries that modify data inside range of this function
    relocations: Vec<relocation::RelocationEntry>,
}
...
enum Global<'a> {
    Copy(wasmparser::Global<'a>),
    ToDefine(GlobalConstructor),
}
...
struct ModuleEmitState<'a> {
    ...
    data: Vec<data_modification::DataSegment<'a>>,
    output_functions: Vec<OutputFunction>,

    // src module
    input_module: InputModule<'a>,
}

impl<'a> ModuleEmitState<'a> {
    pub fn produce_state(
        module_info: analysis::ModuleInfo<'a>,
        ...
        let mut data_to_define = Vec::new();
        data_to_define.extend(output_module_info.included_symbols.iter().filter_map(
            |dep| match dep {
        ...
        data_to_define.sort();
        // it should be not important
        data_to_define.dedup();
        ...
        let mut output_functions: Vec<_> = funcs_to_define
            ...
            .map(|&func_id| {
                ...
                // let relocations = dep_graph.get(&DepNode::Function(func_id))
                //     .into_iter()
                //     .flatten()
                //     .map(|relocation| match {
                //         match relocation {
    ...
    fn generate(&self, output_module: &mut wasm_encoder::Module) -> Result<()> {
        ...
        if main {
            ...
            // not supported
            // self.generate_tags_section(output_module);
        ...
        // self.generate_start_section(output_module);
    ...
    fn generate_tags_section(&self, output_module: &mut wasm_encoder::Module) {
        todo!("Not supported")
    }
    ...
    fn generate_table_element_sections(
        ...
        output_module.section(&section);
        self._generate_element_section(output_module)
    }

    fn _generate_element_section(&self, output_module: &mut wasm_encoder::Module) -> Result<()> {
    ...
    fn generate_memory_section(&self, output_module: &mut wasm_encoder::Module) {
        if self.input_module.memories.is_empty() {
            return;
        }
    ...
    fn generate_global_section(&self, output_module: &mut wasm_encoder::Module) -> Result<()> {
        ...
        for (id, global) in self.globals.iter().enumerate() {
            match global {
                ...
                Global::ToDefine(new) => {
                    section.global(new.global_type(), &new.global_init(lib_base_id));
                }
            }
        }
        output_module.section(&section);
...
//         self.generate_type_section()?; // copy from input
//         self.generate_import_section();// copy from input (remove split point imports)
//         self.generate_function_section(); // use output_functions field
//         self.generate_table_section(); // copy from input
//         self.generate_memory_section(); // copy from input
//         self.generate_global_section(); // use globals field
//         self.generate_export_section(); // 1. for main - copy from input (remove split point exports)
//                                         // 2. for other modules - only keep export section and exports from extracted methods?? (wasm_bindgen?)
//         self.generate_start_section();  // only for main
//         self.generate_element_section()?; // main: copy from input
//         self.generate_data_count_section(); // use data field
//         self.generate_code_section()?; // use output_functions field
//         self.generate_data_section()?;  // use data field
//         self.generate_wasm_bindgen_sections(); // only for main, todo for other
//         self.generate_name_section()?; // generate from output_functions
...
//                 {
//                     println!("Mapping split point {split_point:?} -> {output_func_id}");
