...
pub type DepSet<T = DepNode> = BTreeSet<T>;
pub type DepMiniSet<T = DepNode> = crate::list_set::MiniSet<T>;
...
impl DepGraph {
    ...
    pub fn get_childs(&self, key: &DepNode) -> Option<&DepMiniSet> {
        self.nodes.get(key).map(|s| &s.childs)
    }
    pub fn get_parents(&self, key: &DepNode) -> Option<&DepMiniSet> {
        self.nodes.get(key).map(|s| &s.parents)
    }
    pub fn iter_childs(&self) -> impl Iterator<Item = (&DepNode, &DepMiniSet)> {
...
impl<Id> NamedGraph<Id> {
    ...
    fn collect_visited_by(modules: &[NamedGraph<Id>]) -> HashMap<DepNode, DepSet<usize>> {
        let mut visited_by: HashMap<DepNode, DepSet<usize>> = HashMap::new();
...
mod tests {
    ...
    fn reachablity_graph() {
        ...
        let reachability_graph = super::find_reachable_deps(
            &dep_graph,
            &DepSet::from_iter([DepNode::Function(no_inline_fn)]),
        );
        // no_inline_fn -> data1
        //              -> data2
        //              -> data3
        ...
        let reachability_graph = super::find_reachable_deps(
            &dep_graph,
            &DepSet::from_iter([DepNode::Function(indirect_fn)]),
        );
    ...
    fn test_unique_nodes() {
        ...
        let modules = vec![
            super::NamedGraph::new(
                "module1",
                super::find_reachable_deps(&graph, &testing::uniq_nodes("F(1)").unwrap()),
            ),
            super::NamedGraph::new(
                "module2",
                super::find_reachable_deps(&graph, &testing::uniq_nodes("F(11)").unwrap()),
            ),
        ];
...
F(1) -> F(307)
        ...
        let mut modules = vec![
            ...
            super::NamedGraph::new(
                ...
                super::find_reachable_deps(&graph, &testing::uniq_nodes("F(899)").unwrap()),
            ),
        ];
...
"#;
        ...
        let mut modules = vec![
            ...
            super::NamedGraph::new(
                "mod3",
                super::find_reachable_deps(&graph, &testing::uniq_nodes("F(21)").unwrap()),
            ),
            super::NamedGraph::new(
                "mod4",
                super::find_reachable_deps(&graph, &testing::uniq_nodes("F(31)").unwrap()),
            ),
        ];
    ...
    fn test_extracted_from_example() {
        ...
        let mut modules = vec![
            super::NamedGraph::new(
                "split_static_str",
                super::find_reachable_deps(&graph, &testing::uniq_nodes("F(64)").unwrap()),
            ),
            super::NamedGraph::new(
                "split_string_from_static",
                super::find_reachable_deps(&graph, &testing::uniq_nodes("F(65)").unwrap()),
            ),
            super::NamedGraph::new(
                "main",
                super::find_reachable_deps(&graph, &testing::uniq_nodes("F(2000)").unwrap()),
            ),
        ];
        let shared_entries = super::NamedGraph::calculate_shared_modules(&mut modules, &graph);
