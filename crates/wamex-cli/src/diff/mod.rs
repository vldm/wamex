...
use crate::{
    ...
    metadata::{self, uniq_symbols::GraphNode},
...
impl<'any, 'src> Compare<'any, 'src> {
    ...
    fn print_node_tree_changes(
        old_structure: &crate::metadata::uniq_symbols::ModuleStructure,
        new_structure: &crate::metadata::uniq_symbols::ModuleStructure,
        diff: &crate::metadata::uniq_symbols::DiffResult,
    ) {
    ...
    fn print_cascade_of_changes(
        ...
        for node in added_nodes.iter() {
            if !new_structure.structure.nodes[node]
                ...
                .iter()
                .any(|c| match c {
                    crate::metadata::uniq_symbols::NodeMarker::Lazy { node: child_id, .. } => {
    ...
    fn build_child_tree(
        node_id: &GraphNode,
        structure: &crate::metadata::uniq_symbols::Structure,
        ...
        for child in node.children.iter() {
            let crate::metadata::uniq_symbols::NodeMarker::Lazy { node: child_id, .. } = child
            else {
                continue;
            };
    ...
    ) -> crate::metadata::uniq_symbols::ModuleStructure {
        let info = crate::analysis::ModuleInfo::new(module).unwrap();
        let structure = crate::metadata::_build_module_structure(&info);
        structure
    }
