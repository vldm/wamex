...
use wamex_object::{Module, index::EntityRef};
use wasmparser::{Data, Global};

use crate::{
    analysis::symbols::{DiffEntry, DiffResult},
    index::{IdVec, PrimaryKey},
    read::code::FunctionWithBody,
};
pub struct Compare<'any, 'src> {
    left: &'any Module<'src>,
    right: &'any Module<'src>,
    structural: bool,
}
...
impl<'any, 'src> Compare<'any, 'src> {
    ...
    pub fn print_diff(&self) -> Result<(), anyhow::Error> {
        macro_rules! print_hex_diff {
            ($id:expr, $left: expr, $right: expr) => {
                let num_imports: usize = self.left.functions.items.imports.len();
                let raw_id = $id.index() + num_imports;
                let id = crate::index::FunctionRef::new(raw_id);
                log::info!(
                    "name: {}",
                    self.left
                        .wasm_reader
    ...
    fn print_changed_modules(
        &self,
        differ: &crate::analysis::symbols::Differ<&'any Module<'src>, &'any Module<'src>>,
